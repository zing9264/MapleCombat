...
const TIERS = {
  ...
  '航海師套裝(法師)': {
    2: { mag: 20, boss: 10 },
    3: { all: 30, mag: 20, boss: 10 },
    4: { mag: 25 },
  },
  '永恆套裝(法師)': { 2: { mag: 40, boss: 10 }, 3: { all: 50, mag: 40, boss: 10 } },
  漆黑BOSS套裝: {
    2: { all: 10, mag: 10, boss: 10 },
    3: { all: 10, mag: 10 },
    4: { all: 15, mag: 15 },
    5: { all: 15, mag: 15, boss: 10 },
  },
...
for (const e of items) {
  ...
  if (gem) {
    gemInt += G(e.item_total_option, 'int')
    continue
  }
  itemInt += G(e.item_total_option, 'int')
  itemLuk += G(e.item_total_option, 'luk')
  itemMag += G(e.item_total_option, 'magic_power')
  itemBoss += G(e.item_total_option, 'boss_damage')
  for (const k of [
    'potential_option_1',
    'potential_option_2',
    'potential_option_3',
...
const check = (label, calc, truth) =>
  console.log(
    `${label.padEnd(26)} 推算 ${String(calc).padStart(6)}   遊戲 ${String(truth).padStart(6)}   ${calc === truth ? '✅' : '❌ 差 ' + (truth - calc)}`,
  )
