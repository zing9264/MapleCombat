...
export interface FamiliarLine {
  ...
  name: string
  /**
   * 實際生效的數值，同步時由 API 給（core/familiarImport.ts）。
   *
   * 是該隻當下真正生效的值，不是詞條表上的滿值 —— 暗黑半人馬在 API 裡就是
   * 魔攻 14%、終傷 20%，跟遊戲畫面一致。手動改也可以，用在還沒同步就想先試算。
   *
   * 在草稿裡把一隻在召喚中與羈絆之間搬動時，數值**不會**跟著換算：
   * 那是遊戲端決定的，我們手上只有 API 回報的當下狀態。要看真實結果，
   * 在遊戲裡換好之後重新同步一次。
   */
  value: number
}
